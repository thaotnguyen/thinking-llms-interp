...
# SAEs, activations and responses come from the HF dataset
# matthewshu/medcasereasoning-cot-activations. Set $SAE_SNAPSHOT to a local snapshot dir to
# use an existing copy; otherwise the snapshot is resolved (and fetched into the HF cache on
# first use) via huggingface_hub. Resolution is lazy so --selftest needs no data and no network.
_HF_REPO = "matthewshu/medcasereasoning-cot-activations"
_snapshot_dir = None
...
def snapshot() -> str:
    ...
    if _snapshot_dir is None:
        _snapshot_dir = os.environ.get("SAE_SNAPSHOT")
        if not _snapshot_dir:
            from huggingface_hub import snapshot_download
            _snapshot_dir = snapshot_download(_HF_REPO, repo_type="dataset")
...
def sae_dir():
    return os.path.join(snapshot(), "saes")


def resp_dir():
    return os.path.join(snapshot(), "responses")
