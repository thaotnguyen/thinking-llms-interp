...
def is_thinking_model(model_id: str) -> bool:
    return "reason" in model_id or "deepseek" in model_id
...
async def generate_or_response_async(
    ...
    max_retries: int,
    request_timeout: int,
    get_result_from_response: Callable[[str | tuple[str, str]], Any | None],
...
class ORBatchProcessor(BatchProcessor[BatchItem, BatchResult]):
    def __init__(
        ...
        process_response: Callable[
            [str | tuple[str, str], BatchItem], BatchResult | None
        ],
    ):
        super().__init__(
            model_id=model_id,
        ...
        if is_thinking_model(self.model_id):
            logging.info(f"Working with thinking model {self.model_id}")

        self.max_requests_limit = MAX_OPEN_ROUTER_REQUESTS_LIMIT
        self.request_timeout = 60  # 1 minute
