...
def parse_args():
    ...
    parser.add_argument('--n_cold_start_tokens', type=int, default=5,
...
# NEW: Token-level hybrid generation -------------------------------------------------------------
...
def hybrid_generate_sentence(
    ...
    *,
    ...
    Workflow per iteration (per output token):
        1. Take the hidden activation of the last token produced so far by the
           *thinking* model at `sae_layer` and map it through the SAE to obtain
           the dominant latent feature.
        2. Retrieve its steering vector and (optionally) apply it to the *base*
           model's hidden states to compute steered logits.
        3. If `use_perplexity_guardrail` is set, also compute un-steered logits
           and pick whichever next-token candidate has lower perplexity under
           the thinking model. Otherwise, always use the steered candidate.
    """
    ...
    while generated_tokens < max_new_tokens:
        # ----------------------------------------------------------------------------------
        # (1) THINKING MODEL — use current position to derive steering vector
        # ----------------------------------------------------------------------------------
        with torch.no_grad():
        ...
        # ----------------------------------------------------------------------------------
        # (2) BASE MODEL — generate **one token** with the steering vector (token-level)
        # ----------------------------------------------------------------------------------
