...
get_layers() {
    local model=$1
    case "$model" in
        "deepseek-ai/DeepSeek-R1-Distill-Llama-8B") echo "14" ;; # Total layers: 32
        "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B") echo "16" ;; # Total layers: 28
        "deepseek-ai/DeepSeek-R1-Distill-Qwen-14B") echo "20" ;; # Total layers: 48
        "Qwen/QwQ-32B") echo "27" ;; # Total layers: 64
        ...
        "FreedomIntelligence/HuatuoGPT-o1-8B") echo "18" ;; # Total layers: 32
        "openai/gpt-oss-20b") echo "17" ;; # Total layers: 24
        *) echo "" ;;
    esac
}
...
# for MODEL in $MODELS; do
#     for LAYER in $(get_layers $MODEL); do
#         python generate_titles_trained_clustering.py --model $MODEL --layer $LAYER --clusters $CLUSTERS --n_examples $N_EXAMPLES --clustering_methods $CLUSTERING_METHODS --repetitions $REPETITIONS --command submit --evaluator_model deepseek-chat --max_workers 30
#     done
# done
...
# for MODEL in $MODELS; do
#     for LAYER in $(get_layers $MODEL); do
#         python generate_titles_trained_clustering.py --model $MODEL --layer $LAYER --clusters $CLUSTERS --n_examples $N_EXAMPLES --clustering_methods $CLUSTERING_METHODS --repetitions $REPETITIONS --command process --evaluator_model deepseek-chat --wait-batch-completion
#     done
# done
